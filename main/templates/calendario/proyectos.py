    ...
    if request.method == 'POST':
        if 'desactivar_usuario_proyecto' in request.form:
            ...
            if usuario_proyecto is not None:
                ...
                params = [generarID(),project_id, id_usuario, fecha_user, observaciones_user_proyecto]
                cursor.execute(query, params)
                conexion.commit()
...
def misProyectos():
    ...
    conexion = mysql.connect()
    cursor = conexion.cursor()
    cursor.execute("SELECT proyectos.*  FROM proyecto_users LEFT JOIN proyectos ON  proyecto_users.id_proyecto=proyectos.id_proyecto WHERE id_usuario=%s;", session['usuario'])
...
def verProyectoEliminar():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('borrar_proyecto'):
            ...
            ruta_archivo = os.path.join(app.root_path, 'static', 'images', 'proyectos', nombre_archivo[0])
            cursor.execute(
...
def crearProyecto():
    ...
    if request.method == 'POST' and 'crear_proyecto' in request.form:
        ...
        params = [generarID(),nombre_proyecto, nuevoNombreImagen, descripcion_proyecto]
