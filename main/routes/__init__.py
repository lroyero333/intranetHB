...
def notificacionesRH():
    ...
    cursor.execute("SELECT notificaciones.*, general_users.Nombre, general_users.Apellido, general_users.foto FROM notificaciones LEFT JOIN general_users ON notificaciones.creador_solicitud= general_users.usuario WHERE id_usuario= %s and estado_notificacion= %s ORDER BY fecha_notificacion DESC;", (session['usuario'], "No visto"))
    ...
    if tipo_solicitud == "Nomina":
        ...
        if request.method == 'POST':
            if 'aceptarSolicitudNomina' in request.form:
                ...
                params = [generarID(),tipo_notificacion, user_nomina, id_solicitud, persona_resuelve_solicitud, mensaje, fechaResolucion]
                cursor.execute(query, params)
                conexion.commit()
        ...
        if request.method == 'POST':
            if 'aceptarSolicitudVacaciones' in request.form:
                ...
                cursor.execute(query, params)
                print('HOLAHLAHL9AFFFFFFFFFFF')

                conexion.commit()
                query = "INSERT INTO notificaciones (id_notificacion, tipo_notificacion, id_usuario, id_solicitud, creador_solicitud ,mensaje, fecha_notificacion) VALUES (%s, %s,%s,%s,%s,%s,%s)"
                params = [generarID(),tipo_notificacion, user_vacaiones, id_solicitud, persona_resuelve_solicitud, mensaje, fechaResolucion]
                cursor.execute(query, params)
                conexion.commit()
        ...
        if request.method == 'POST':      
            if 'aceptarSolicitudCertificado' in request.form:
                ...
                conexion.commit()
                query = "INSERT INTO notificaciones (id_notificacion, tipo_notificacion, id_usuario, id_solicitud, creador_solicitud ,mensaje, fecha_notificacion) VALUES (%s, %s,%s,%s,%s,%s,%s)"
                params = [generarID(),tipo_notificacion, user_certificate, id_solicitud, persona_resuelve_solicitud, mensaje, fechaResolucion]
                cursor.execute(query, params)
                conexion.commit()
        ...
        if request.method == 'POST':
            if 'aceptarSolicitudPermiso' in request.form:
                ...
                conexion.commit()
                query = "INSERT INTO notificaciones (id_notificacion, tipo_notificacion, id_usuario, id_solicitud, creador_solicitud ,mensaje, fecha_notificacion) VALUES (%s, %s,%s,%s,%s,%s,%s)"
                params = [generarID(),tipo_notificacion, user_permiso, id_solicitud, persona_resuelve_solicitud, mensaje, fechaResolucion]
                cursor.execute(query, params)
                conexion.commit()
        ...
        if request.method == 'POST':
            if 'aceptarSolicitudInventario' in request.form:
                ...
                params = [persona_resuelve_solicitud, fechaResolucion,estado_solicitud, observaciones,id_solicitud]
                cursor.execute(query, params)
                conexion.commit()
                query = "INSERT INTO notificaciones (id_notificacion, tipo_notificacion, id_usuario, id_solicitud, creador_solicitud ,mensaje, fecha_notificacion) VALUES (%s, %s,%s,%s,%s,%s,%s)"
                params = [generarID(),tipo_notificacion, user_inventario, id_solicitud, persona_resuelve_solicitud, mensaje, fechaResolucion]
                cursor.execute(query, params)
                conexion.commit()
                flash('Respuesta realizada correctamente','correcto')
